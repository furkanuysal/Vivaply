...
export default function MediaCard({ content, type }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Bilgiler */}
      <div className="p-4">
        <h3 className="text-white font-bold truncate" title={content.display_name}>
          {content.display_name}
        </h3>
        <div className="flex justify-between items-center mt-2 text-xs text-gray-400">
          <span>{content.display_date?.split('-')[0] || 'Tarih Yok'}</span>
          <span className="uppercase border border-gray-600 px-1 rounded">{type}</span>
        </div>
      </div>
    </div>
  );
